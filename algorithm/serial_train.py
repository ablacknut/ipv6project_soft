...
def train(_vali_rate, batch_size, epochs, featuredim):
    ...
    epoch = epochs
    T_fea = 'T7'
    save_model_file = "models/" + T_fea + 'best_MUlit140_train.h5'
    ...
    def MultiHeadsAttModel(l=8 * 8, d=512, dv=64, dout=512, nv=8):
        ...
        att = Lambda(lambda x: K.batch_dot(x[0], x[1], axes=[-1, -1]) / np.sqrt(dv), output_shape=(l, nv, nv))([q,
                                                                                                                k])  # l, nv, nv
    ...
    data = pad_sequences(sequences, maxlen=MAX_SEQUENCE_LENGTH)  #, maxlen=MAX_SEQUENCE_LENGTH
    ...
    y_pred_class = model.predict([np.array(x_val2)]).reshape(len(y_val2))
    #将y_val变为list类型             重要！！！！！！！！！！！！！！
