...
class IndRNNCell(Layer):
    def __init__(self,
        ...
        if implementation != 2:
            warnings.warn("IndRNN only supports implementation 2 for the moment. Defaulting to implementation = 2")
            implementation = 2
    ...
    def build(self, input_shape):
        ...
        if self.recurrent_initializer is None:
            if self.recurrent_clip_min is not None and self.recurrent_clip_max is not None:
                ...
                self.recurrent_initializer = initializers.uniform(-initialization_value, initialization_value)
            else:
        ...
        if self.recurrent_clip_min is not None and self.recurrent_clip_max is not None:
            ...
            self.recurrent_kernel = K.clip(self.recurrent_kernel, self.recurrent_clip_min, self.recurrent_clip_max)
    ...
    def call(self, inputs, states, training=None):
        if 0 < self.dropout < 1 and self._dropout_mask is None:
            self._dropout_mask = _generate_dropout_mask(_generate_dropout_ones(inputs,
                                                                               K.shape(inputs)[-1]),
                                                        self.dropout,
                                                        training=training,
                                                        count=1)
        if (0 < self.recurrent_dropout < 1 and self._recurrent_masks is None):
            _recurrent_mask = _generate_dropout_mask(_generate_dropout_ones(inputs, self.units),
...
class IndRNN(RNN):
    def __init__(self,
        ...
        if K.backend() == 'theano':
            warnings.warn('RNN dropout is no longer supported with the Theano backend '
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None, constants=None):
        ...
        return super(IndRNN, self).call(inputs, mask=mask, training=training, initial_state=initial_state, constants=constants)
